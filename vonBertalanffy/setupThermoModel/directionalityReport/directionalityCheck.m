        ...
        if max(GB(g,:))==1
            ...
            for n=1:nRxn
                if GB(g,n)==1
                    ...
                    for p=1:length(sInd)
                        %                 fprintf(fid,'\t\t%i\t\t%6.2f\t\t%6.2f\t%6.2f\t\t%i\t%s%s\n',model.S(sInd(p),n),model.met(sInd(p)).dGft0,rt*log(model.met(sInd(p)).concMin),rt*log(model.met(sInd(p)).concMax),sInd(p),model.met(sInd(p)).officialName,[' ' model.met(sInd(p)).abbreviation(end-2:end)]);
                        fprintf(fid,'%i\t%8g\t%8.2f\t%6.2f\t%6.2f\t%20s\t%s\n',sInd(p),model.S(sInd(p),n),model.met(sInd(p)).dGft0,rt*log(model.met(sInd(p)).concMin),rt*log(model.met(sInd(p)).concMax),model.met(sInd(p)).abbreviation,model.met(sInd(p)).officialName);
                    end
                    fprintf(fid,'%s\n','products:');
...
if printToTable
    ...
    for g=1:size(GB,1)
        ...
        allMetSum=sum(abs(model.S(:,GB(g,:))),2);
        for m=1:nMet
