function model=assignThermoToModel(model,Alberty2006,temp,PHR,IS,CHI,compartments,NaNdGf0GCMetBool,Legendre,LegendreCHI,useKeqData,printToFile)
...
%in the network
%fixed in setCommonZeroStandardGibbsEnergyOfFormation.m
%NO EXCEPTIONS AT THE MOMENT BUT ENTER THE MODEL ABBR's OF EXCEPTIONS HERE
%IF NECESSARY
exceptions={''};
...
%     end
    ...
    if albertyMatch
        ...
        if NaNdGf0GCMetBool(m)
            ...
            if any(strcmp(model.met(m).abbreviation(1:end-3),exceptions))
                %in exceptional cases, use group contribution estimates over estimates
                %from Equilibrium constants. see line 80 for list of exceptions
