function model=deltaG0concFluxConstraintBounds(model,Legendre,LegendreCHI,figures,nStdDevGroupCont)
...
%             end
            ...
            dGft0Max(1,m)=model.met(m).dGft0+errorKeq*abs(model.met(m).dGft0);
...
nanRxn=sum(abs(model.S(nan_dGformationBool,:)),1)~=0; 
...
dGt0Max=dGft0Min*Sneg+dGft0Max*Spos;
dGt0Min=dGft0Max*Sneg+dGft0Min*Spos;
%min and max milimolar standard change in chemical potential
dGtmMMax=dGftmMMin*Sneg+dGftmMMax*Spos;
dGtmMMin=dGftmMMax*Sneg+dGftmMMin*Spos;
%min and max change in chemical potential
dGtMax=dGftMin*Sneg+dGftMax*Spos;
dGtMin=dGftMax*Sneg+dGftMin*Spos;
...
for n=1:nRxn
    if model.transportRxnBool(n)
        ...
        for p=1:length(metAbbrAllShort)
            ...
            if nnz(strcmp(metAbbrAllShort{p},metAbbrAllShort))>1 &&  strcmp(model.met(metBool).dGft0Source,'GC')
                ...
                if isnan(model.met(metBool).dGft0)
                    ...
                    dGftMinTemp(metBool)=model.met(metBool).dGft + rt*log(concMin(metBool));
                    dGftMaxTemp(metBool)=model.met(metBool).dGft + rt*log(concMax(metBool));
                end
            end
        end
        ...
        if fix
            ...
            dGt0Max(n)=dGft0MinTemp*Sneg(:,n)+dGft0MaxTemp*Spos(:,n);
            dGt0Min(n)=dGft0MaxTemp*Sneg(:,n)+dGft0MinTemp*Spos(:,n);
            %thor standard
            dGtmMMax(n)=dGftmMMinTemp*Sneg(:,n)+dGftmMMaxTemp*Spos(:,n);
            dGtmMMin(n)=dGftmMMaxTemp*Sneg(:,n)+dGftmMMinTemp*Spos(:,n);
            %
            dGtMax(n)=dGftMinTemp*Sneg(:,n)+dGftMaxTemp*Spos(:,n);
            dGtMin(n)=dGftMaxTemp*Sneg(:,n)+dGftMinTemp*Spos(:,n);
        end
   end
end
