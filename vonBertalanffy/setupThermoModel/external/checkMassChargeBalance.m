function [imBalancedMass,imBalancedCharge,imBalancedBool] = checkMassChargeBalance(model,rxnBool,printLevel)
...
for j = 1 : length(Elements)
    if j==1
        ...
        %formula
        [bal(:,j),E(:,j)]=checkBalance(model,Elements{j},0);
        fprintf('%s\n',['Checking element ' Elements{j}]);
    end
end
imBalancedBool=~(sum(abs(bal'))'==0);
        ...
        for j = 1 : length(Elements)
            if bal(i,j)~=0
                if ~strcmp(imBalancedMass{i,1},'')
                    imBalancedMass{i,1} = [imBalancedMass{i,1} ', ' int2str(bal(i,j)) ' ' Elements{j}];
                else
                    imBalancedMass{i,1} = [int2str(bal(i,j)) ' ' Elements{j}];
                end
            end
...
if printLevel==-1
    ...
    if ~isempty(imBalancedCharge)
        for q=1:nRxn
            if model.SIntRxnBool(q) & dC(q)~=0
                if ~firstMissing
                    fid=fopen('charge_imbalanced_reactions.txt','w');
                    warning('There are charged imbalanced reactions, see charge_imbalanced_reactions.txt')
                    firstMissing=1;
                end
...
if printLevel==1
    if ~isempty(imBalancedCharge) & ~strcmp(imBalancedMass{p,1},'') & ~strcmp(imBalancedMass{p,1},'NaN')
        for q=1:nRxn
            if model.SIntRxnBool(q) & dC(q)~=0
                equation=printRxnFormula(model,model.rxns(q),0);
                fprintf('%s\t%s\t%s\n',int2str(q),model.rxns{q},equation{1});
