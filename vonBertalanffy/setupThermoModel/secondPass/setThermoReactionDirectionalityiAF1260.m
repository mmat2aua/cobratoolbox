function [modelD,solutionThermoRecon,solutionRecon,model1]=setThermoReactionDirectionalityiAF1260(model,directions,maxFlux,hardCoupleOxPhos)
...
if strcmp(CBTLPSOLVER,'cplex_direct')
    ...
    primalOnlyFlag=[];
    solver=CBTLPSOLVER;
    model.A=model.S; model.osense = -1;
    %solutionRecon = solveCobraLP(model,primalOnlyFlag,solver,printLevel);
    solutionRecon = solveCobraLP(model); % changed by Jan Schellenberger - header to solveCobraLP changed.  printLevel must be set through changeCobraSolverParams
    model=rmfield(model,'A');model=rmfield(model,'osense');
end
fprintf('\n%s\t%g\n','Growth with reconstruction directions: ',solutionRecon.obj);
...
if strcmp(CBTLPSOLVER,'cplex_direct')
    ...
    solution = solveCobraLP(modelD,primalOnlyFlag,solver,printLevel);
    modelD=rmfield(modelD,'A');modelD=rmfield(modelD,'osense');
end
fprintf('\n%s\t%g\n','Growth with thermo & recon directions: ',solution.obj);
...
if abs(solution.obj)<abs(solutionRecon.obj/10) || solution.origStat~=1
    ...
    if runThrough
        for n=1:nRxn
            if thermoReverseNeededForwardBool(n) || thermoForwardNeededReverseBool(n)
                ...
                fprintf('%s%70s%s%g\n','Changed ' , modelD.rxn(n).officialName,'. Growth: ',solution.obj);
            end
        end
        fprintf('\n');
        ...
        for n=1:nRxn
            if thermoReverseNeededForwardBool(n) || thermoForwardNeededReverseBool(n)
                ...
                fprintf('%s%70s%s%g\n','Changed also ' , modelD.rxn(n).officialName,'. Growth: ',solution.obj);
            end
        end
        fprintf('\n');
    end
    ...
    if dGt0MeanCutoff~=Inf;
        ...
        modelD.rxn(strcmp(modelD.rxns,'HISTP')).directionalityThermo='forward';
        ...
        modelD.rxn(strcmp(modelD.rxns,'THDPS')).directionalityThermo='forward';
        ...
        modelD.rxn(strcmp(modelD.rxns,'GLCP')).directionalityThermo='forward';

        %   officialName: 'glycogen phosphorylase'
        %   equation: 'bglycogen[c] + pi[c]  -> g1p[c] '
        modelD.lb(strcmp(modelD.rxns,'GLCP2'))=0;
        modelD.ub(strcmp(modelD.rxns,'GLCP2'))=maxFlux;
        modelD.rxn(strcmp(modelD.rxns,'GLCP2')).directionalityThermo='forward';
    end
    ...
    modelD.rxn(strcmp(modelD.rxns,'METAT')).directionalityThermo='forward';
    ...
    modelD.rxn(strcmp(modelD.rxns,'MECDPS')).directionalityThermo='forward';
    ...
    modelD.rxn(strcmp(modelD.rxns,'IMPC')).directionalityThermo='forward';
    ...
    modelD.rxn(strcmp(modelD.rxns,'ATPM')).directionalityThermo='forward';
    ...
    fprintf('%s%g\n','Reconstruction directions for essential reactions. Growth: ',solution.obj);
end
...
fprintf('\n%s%g\n','Reactions driven by cofactors set to irreversible. Growth: ',solution.obj);
...
%     clear modelTransportRecon
    ...
    primalOnlyFlag=[];
    solver=CBTLPSOLVER;
    modelTransportRecon.A=modelTransportRecon.S; modelTransportRecon.osense=-1;
    solutionTransportRecon = solveCobraLP(modelTransportRecon,primalOnlyFlag,solver,printLevel);
%     clear modelTransportRecon
end
fprintf('\n%s%g\n','All transport reactions set to reconstruction directions. Growth: ',solution.obj);
...
for n=1:nRxn
    if strcmp(modelD.rxn(n).directionality,'forward') && solutionTransportRecon.full(n)<-0.1
        fprintf('%15s\t%4.2f\t%4.2f\t%65s\t%s\n',modelD.rxns{n},solutionRecon.full(n),solutionTransportRecon.full(n),modelD.rxn(n).equation,modelD.rxn(n).officialName);
    end
end
...
if strcmp(CBTLPSOLVER,'cplex_direct')
    ...
    primalOnlyFlag=[];
    solver=CBTLPSOLVER;
    modelTransportReconHardC.A=modelTransportReconHardC.S; modelTransportReconHardC.osense=-1;
    solutionTransportReconHardC = solveCobraLP(modelTransportReconHardC,primalOnlyFlag,solver,printLevel);
    clear modelTransportReconHardC
end
if hardCoupleOxPhos
    ...
    for p=1:size(pair,1)
        fprintf('%s\t%6.4g\t\t%s\t%6.4g\n',pair{p,1},solutionTransportReconHardC.full(strcmp(pair{p,1},modelD.rxns)),pair{p,2},solutionTransportReconHardC.full(strcmp(pair{p,2},modelD.rxns)));
    end
end
fprintf('\n%s%g\n','All transport with reconstruction directions, plus hard coupling ox phos. Growth: ',solutionTransportReconHardC.obj);
...
for n=1:nRxn
    if strcmp(modelD.rxn(n).directionality,'forward') && solution.full(n)<-0.1
        fprintf('%15s\t%4.2f\t%4.2f\t%65s\t%s\n',modelD.rxns{n},solutionRecon.full(n),solution.full(n),modelD.rxn(n).equation,modelD.rxn(n).officialName);
    end
end
fprintf('\n%s%g\n','Certain transport reactions set to irreversible. Growth: ',solution.obj);
fprintf('\n')
...
if hardCoupleOxPhos
    ...
    fprintf('\n%s%g\n','Certain transport reactions set to irreversible * hard coupling ox phos. Growth: ',solution.obj);
    fprintf('\n')
...
if hardCoupleOxPhos
    fprintf('\n%s%g\n','Certain transport & Hard Coupling & other reactions irreversible. Growth: ',solution.obj);
else
    fprintf('\n%s%g\n','Certain transport & other reactions irreversible. Growth: ',solution.obj);
end
...
if strcmp(CBTLPSOLVER,'cplex_direct')
    ...
    solver=CBTLPSOLVER;
    modelD.A=modelD.S; modelD.osense=-1;
    solutionThermoRecon = solveCobraLP(modelD,primalOnlyFlag,solver,printLevel);
    modelD=rmfield(modelD,'A');modelD=rmfield(modelD,'osense');
end
...
if hardCoupleOxPhos
    ...
    %phosphorylation reactions
    for p=1:size(pair,1)
        fprintf('%s\t%6.4g\t\t%s\t%6.4g\n',pair{p,1},solutionThermoRecon.full(strcmp(pair{p,1},modelD.rxns)),pair{p,2},solutionThermoRecon.full(strcmp(pair{p,2},modelD.rxns)));
    end
end

fprintf('\n%s%g\n','Final ThermoRecon growth: ',solutionThermoRecon.obj);
fprintf('%s\t%g\n','c.f. growth with reconstruction directions: ',solutionRecon.obj);
...
if hardCoupleOxPhos
    ...
    for p=1:length(oxPhos)
        fprintf('%15s\t%4.2f\t%4.2f\t%65s\t%s\n',oxPhos{p},solutionRecon.full(strcmp(oxPhos{p,1},modelD.rxns)),solutionThermoRecon.full(strcmp(oxPhos{p,1},modelD.rxns)),modelD.rxn(strcmp(oxPhos{p,1},modelD.rxns)).equation,modelD.rxn(strcmp(oxPhos{p,1},modelD.rxns)).officialName);
    end
end
