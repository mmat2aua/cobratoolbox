function model=setCommonZeroStandardGibbsEnergyOfFormation(model)
...
for p=1:length(uniqueCompartments)
    ...
    coAset={'coa','aacoa','accoa','mmcoa-R','mmcoa-S','ppcoa','succoa','malcoa'};
    leftAll{n}=[];n=n+1;
    ...
    %NOTE: some cofactors in the pairs listed above are not listed in Albertys
    %tables e.g. mmcoa-S
    ...
    fprintf('\n%s\n%s\n','Setting all exceptional Alberty reactants to have a common thermodynamic',...
    ...
    while x<=length(leftAll)
        ...
        if any(strcmp(leftAll{x},model.mets)) && any(strcmp(rightAll{x},model.mets))
            abbrL=leftAll{x};
            abbrR=rightAll{x};
            %abbrL=abbrL(1:end-3);
            %abbrR=abbrR(1:end-3);
            ...
            if ~(any(strcmp(abbrL,coAset) | any(strcmp(abbrR,coAset))))
                ...
                if nCoLeftOnly==0 && nCoRightOnly~=0
                    ...
                    for m=1:nMet
                        abbr=model.mets{m};
                        abbr=abbr(1:end-3);
                        if any(strcmp(abbr,{abbrL,abbrR}))
                ...
                if nCoLeftOnly~=0 && nCoRightOnly==0
                    ...
                    if strcmp(leftAll{x},['nad' uniqueCompartments{p}]) && strcmp(rightAll{x},['nadh' uniqueCompartments{p}]) && any(strcmp(model.mets,['nmn' uniqueCompartments{p}] ))
                        fprintf('%s\n',['Assuming Albertys value for nmn is set to nad baseline']);
                        for m=1:nMet
                            %abbr=model.mets{m};
                            %abbr=abbr(1:end-3);
                            if any(strcmp(abbr,'nmn'))
                                %only change if we are using Albertys data
                                if strcmp(model.met(m).dGft0Source,'Keq')
                                    model.met(m).dGft0=model.met(m).dGft0+baseLineChange;
                ...
                if coaSorted==0
                    for s=1:length(coAset)
                        baseLineChange=model.met(strcmp(model.mets,'coa[c]')).dGft0GroupCont-model.met(strcmp(model.mets,'coa[c]')).dGft0Keq;
                        for m=1:nMet
                            %abbr=model.mets{m};
                            %abbr=abbr(1:end-3);
                            if strcmp(coAset{s},abbr)
                                %only change if we are using Albertys data
                                if strcmp(model.met(m).dGft0Source,'Keq')
                                    model.met(m).dGft0=model.met(m).dGft0+baseLineChange;
                                end
...
for x=1:length(leftAll)
    if isempty(leftAll{x})
        ...
        if any(strcmp(model.mets,leftAll{x}))  && any(strcmp(model.mets,rightAll{x}))
            fprintf('%20s\t\t%i\t\t\t%i\t\t%i\t\t%i\t\t%i\t\t%8.4g\t\t%8.4g\n',[leftAll{x} '-' rightAll{x}],nCoFcLRxn,nCoFcRRxn,nCoFc2Rxn,nCoLeftOnly,nCoRightOnly,...
                model.met(strcmp(model.mets,leftAll{x})).dGft0Keq-model.met(strcmp(model.mets,rightAll{x})).dGft0Keq,...
                model.met(strcmp(model.mets,leftAll{x})).dGft0-model.met(strcmp(model.mets,rightAll{x})).dGft0);
        end
    end
end
