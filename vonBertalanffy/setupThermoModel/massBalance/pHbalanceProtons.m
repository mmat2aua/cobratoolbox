function model=pHbalanceProtons(model,imBalancedBool)
...
for n=1:nRxn
    if strcmp('SUCCt2_2',model.rxns{n})
        pause(eps)
    end

    %no change for biomass reaction or exchange reactions or imbalanced
    %reactions
    if model.SIntRxnBool(n) && ~imBalancedBool(n)
        ...
        if ~any(isnan(aveHbound(model.S(:,n)~=0)))
            ...
            if any(model.S(:,n)~=0 & ~boolH)
                ...
                uniqueMetCompartments=unique(metCompartments);
                ...
                if length(uniqueMetCompartments)>length(uniqueCompartmentsNotH)
